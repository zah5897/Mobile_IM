...
import com.mobile.im.LocationManger;
import com.mobile.im.R;
import com.mobile.im.location.utils.MarkerUtil;
import com.mobile.im.location.utils.NotificationUtils;
...
import im.mobile.event.LocationEvent;
...
public class MapActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        to = getIntent().getStringExtra("username");
        initViews();
...
//        LocationManger.getLocationManger().loadPublish(Integer.MAX_VALUE, 20, new Callback() {
//            @Override
...
//                            addMarker(obj.getJSONArray("data"));
...
//    public void addMarker(JSONArray locations) {
//        for (int i = 0, len = locations.length(); i < len; i++) {
//            try {
//                JSONObject lo = locations.getJSONObject(i);
//                MarkerUtil.initLocationMarkInfos(mMapView, mBaiduMap, lo);
//            } catch (JSONException e) {
//                e.printStackTrace();
//            }
//        }
//    }
    ...
    public void addMarker(LocationEvent event) {
        mBaiduMap.clear();
        MarkerUtil.initLocationMarkInfos(mMapView, mBaiduMap, event.lat, event.lng, event.username);
    }
...
//        mBaiduMap.setMyLocationEnabled(true);
        ...
        mBaiduMap.setOnMarkerClickListener(new BaiduMap.OnMarkerClickListener() {
            ...
            public boolean onMarkerClick(Marker marker) {
                Toast.makeText(getApplicationContext(), marker.getExtraInfo().getString("username"), Toast.LENGTH_SHORT).show();
                return false;
            }
        });
...
//            mBaiduMap.addOverlay(dotOption);
            ...
            if (TextUtils.isEmpty(to)) {
                LocationManger.getLocationManger().publishLocationOnly(getApplicationContext(), bdLocation);
            } else {
                LocationManger.getLocationManger().publishLocationToUser(getApplicationContext(), bdLocation, to);
            }
